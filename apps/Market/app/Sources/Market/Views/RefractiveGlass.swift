...
enum Refractive {
    ...
    /// Cool white. The glass separates from a dead-black ground by temperature
    /// rather than by brightness, which is what lets the canvas stay black.
    ...
    /// The ground a slab sits on. Dead black with a single overhead glow — no
    /// colour field, because colour in the canvas fights the content.
    static let canvasDark = Color(red: 0.020, green: 0.020, blue: 0.027)
...
/// appearance this is dead black with a single overhead glow — the cool
/// separation lives in the material, not in the background, which is why the
/// canvas can afford to carry no colour at all. In light appearance the system
/// window background already does that job, but the light still hangs here.
...
private struct RefractiveCanvas: ViewModifier {
    ...
    /// Dark-only apps keep the black ground regardless of system appearance.
    let forceDark: Bool
