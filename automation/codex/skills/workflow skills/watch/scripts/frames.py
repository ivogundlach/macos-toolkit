...
def format_time(seconds: float) -> str:
    ...
    minutes = int(rem // 60)
    sec = rem - minutes * 60
    sec_text = f"{int(sec):02d}" if sec.is_integer() else f"{sec:04.1f}"
    if hours:
...
def merge_frames(primary: list[dict], pinned: list[dict]) -> list[dict]:
    ...
    combined = sorted(
        [*primary, *pinned],
        key=lambda frame: (frame["timestamp_seconds"], frame.get("reason") != "transcript-cue"),
    )
    merged: list[dict] = []
    for frame in combined:
        if merged and abs(frame["timestamp_seconds"] - merged[-1]["timestamp_seconds"]) <= 0.25:
            keep = merged[-1]
            if frame.get("reason") == "transcript-cue" and keep.get("reason") != "transcript-cue":
                merged[-1] = frame
                discard = keep
            else:
                discard = frame
            try:
                Path(discard["path"]).unlink()
            except OSError:
                pass
            continue
        merged.append(frame)
...
def extract_scene_or_uniform(
    ...
    return frames, {
        ...
        "fallback_from_candidates": scene_count,
        "deduped_count": n_dropped,
...
def extract_keyframes(
    ...
    if len(candidates) < KEYFRAME_MIN:
        ...
        return frames_out, {
            ...
            "fallback_from_candidates": len(candidates),
            "deduped_count": n_dropped,
