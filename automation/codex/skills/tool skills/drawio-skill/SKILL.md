---
name: drawio-skill
description: Use for structured node-and-edge diagrams exported as files (PNG/SVG/PDF/JPG) — flowcharts, architecture diagrams, ER diagrams, UML/sequence/class diagrams, network topology, ML/DL model figures (Transformer/CNN/LSTM), mind maps — especially when they need custom styling, rich shape vocabulary, or swimlanes. Generates .drawio XML and exports locally via the native draw.io desktop CLI. Not for raster/photo images (use imagegen) or data charts/plots (use dataviz).
license: MIT
metadata:
  openclaw:
    requires:
...
After self-check, show the exported image. Do not interrupt every deterministic diagram request for feedback. By default, create the editable `.drawio` file plus the requested export format when specified, then report the result. Use an AskUserQuestion-style prompt only when the requested export format, visual style, or review mode is genuinely ambiguous, or when the user explicitly asked for collaborative review. If the current agent surface supports a native ask-user popup, use it; otherwise ask one plain-English question in chat.
