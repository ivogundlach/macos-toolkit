...
# using Codex Luna at max reasoning to capture real end-to-end output.

SKILL_DIR="${LAST30DAYS_CODEX_SKILL_DIR:-$HOME/.codex/skills/tool skills/last30days}"
...
run_version() {
  ...
  for i in "${!QUERIES[@]}"; do
    ...
    # Run Codex Luna at max reasoning with the skill invocation.
    # No timeout — Codex exits on its own; kill manually if stuck.
...
Have Codex read all 34 output files and generate scored comparison:
```
