...
def _propagate_config_to_environ(config: dict[str, object]) -> None:
    """Expose configured retrieval endpoint overrides to source clients."""
...
def main() -> int:
    ...
    if topic.lower() == "setup":
        ...
        # FROM_BROWSER unset so the default Safari-then-Firefox sequence remains
        # available. We deliberately do NOT pin "auto" here (it would re-probe
    ...
    try:
        ...
        if args.plan:
            ...
            try:
                ...
                # Fail fast instead of silently dropping to a weaker deterministic
                # plan and burning a source run the user did not ask for. Mirrors the
