...
_EXA_CLI = Path(__file__).resolve().parents[3] / "exa-search" / "scripts" / "exa_cli.py"
_EXA_HTTP_TIMEOUT = 20
_EXA_ATTEMPTS = 2
# Two HTTP attempts, one bounded backoff, and process startup/JSON margin.
_EXA_PROCESS_TIMEOUT = (_EXA_HTTP_TIMEOUT * _EXA_ATTEMPTS) + 12


def _redact_exa_error(value: str, api_key: str) -> str:
    sanitized = value or ""
    if api_key:
...
def _exa_error_message(result: subprocess.CompletedProcess[str], api_key: str) -> str:
    for raw in (result.stderr, result.stdout):
        sanitized = _redact_exa_error(raw, api_key).strip()
        if not sanitized:
            continue
        try:
        ...
        if isinstance(payload, dict):
            kind = payload.get("kind") or "exa_error"
            message = payload.get("message") or "Exa search failed"
            repair = payload.get("repair") or "Run `exa-search status --live` and retry."
            return f"{kind}: {message} Repair: {repair}"
    return f"exa_cli_exit_{result.returncode}: Exa search failed without diagnostic output"

def exa_search(
    query: str, date_range: tuple[str, str], api_key: str, count: int = 5,
) -> tuple[list[dict], dict]:
    if not _EXA_CLI.is_file():
        raise RuntimeError(
            f"Canonical Exa CLI is missing at {_EXA_CLI}. "
            "Restore the exa-search skill and run `exa-search status --live`."
        )
    command = [
        sys.executable,
        str(_EXA_CLI),
        "--timeout", str(_EXA_HTTP_TIMEOUT),
        "--attempts", str(_EXA_ATTEMPTS),
        "search", query,
        "--type", "auto",
        "--num-results", str(count),
        "--content", "text",
        "--max-characters", "2000",
        "--start-published", f"{date_range[0]}T00:00:00.000Z",
        "--end-published", f"{date_range[1]}T23:59:59.999Z",
    ]
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=_EXA_PROCESS_TIMEOUT,
            check=False,
            env=child_env,
        )
    except subprocess.TimeoutExpired as error:
        output = _redact_exa_error(
            f"{error.stdout or ''}\n{error.stderr or ''}", api_key,
        ).strip()
        detail = f" Last output: {output}" if output else ""
        raise RuntimeError(
            f"Canonical `exa-search` timed out after {_EXA_PROCESS_TIMEOUT}s.{detail} "
            "Run `exa-search status --live`, then retry the Last30Days request."
        ) from error
    except OSError as error:
        raise RuntimeError(
            f"Canonical `exa-search` could not start: {error}. "
            "Restore the exa-search skill and verify its Python runtime."
        ) from error
    if result.returncode != 0:
        raise RuntimeError(
            f"Canonical `exa-search` failed: {_exa_error_message(result, api_key)}"
        )
    try:
        envelope = json.loads(result.stdout)
        data = envelope["response"]
    except (json.JSONDecodeError, KeyError, TypeError) as error:
        diagnostic = _redact_exa_error(result.stdout or result.stderr, api_key)
        raise RuntimeError(
            "Canonical `exa-search` returned invalid structured output. "
            f"Output: {diagnostic} Repair: run `exa-search status --live` and retry."
        ) from error
    if not isinstance(data, dict):
        raise RuntimeError(
            "Canonical `exa-search` returned a non-object response. "
            "Run `exa-search status --live` and retry."
        )
    items = []
