...
def run_auto_setup(config: Dict[str, Any], *, allow_browser_cookies: bool = True) -> Dict[str, Any]:
    """Perform the auto-setup actions.

    - Runs cookie extraction for all registered domains by default, trying the
      browsers from ``env.cookie_extraction_browsers()``. Pass
      ``allow_browser_cookies=False`` for the explicit no-cookie path.
    ...
    if allow_browser_cookies:
        ...
        if not (cookie_config.get("FROM_BROWSER") or "").strip():
            cookie_config["FROM_BROWSER"] = "safari,firefox"
...
def write_setup_config(env_path: Path, from_browser: str | None = None) -> bool:
    ...
    Args:
        ...
        from_browser: Browser extraction mode to persist. Pass the browser that
            ...
            default (Safari/Firefox, no Chromium Keychain prompt) then applies. We avoid
