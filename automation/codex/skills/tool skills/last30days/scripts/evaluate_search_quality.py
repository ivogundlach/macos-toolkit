...
def get_judgments(
    *,
    ...
    if not items:
        return {}
    prompt_dir = output_dir / "judgment-prompts"
    prompt_dir.mkdir(parents=True, exist_ok=True)
    (prompt_dir / f"{slug}.txt").write_text(build_judge_prompt(topic, query_type, items) + "\n")
    if judgments_dir is None:
        sys.stderr.write(
            f"[Eval] No host judgments supplied for {slug!r}; wrote a judgment "
            "prompt and left judged metrics ungraded.\n"
        )
        return {}
    judgment_file = judgments_dir / f"{slug}.json"
    if not judgment_file.exists():
        sys.stderr.write(
            f"[Eval] Host judgment file is missing: {judgment_file}; judged metrics "
            "for this topic will be ungraded.\n"
        )
        return {}
    payload = json.loads(judgment_file.read_text())
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--judgments-dir",
        help="Directory of host-produced <topic-slug>.json judgment files; no external judge is called.",
    )
...
def main() -> int:
    ...
    judgments_dir = Path(args.judgments_dir).resolve() if args.judgments_dir else None
