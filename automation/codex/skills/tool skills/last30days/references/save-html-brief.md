...
- [When to fire this flow](#when-to-fire-this-flow)
- [How to fire it](#how-to-fire-it)
- [Optional hosted publishing](#optional-hosted-publishing)
- [Chat handoff after saving](#chat-handoff-after-saving)
- [What ends up in the HTML file](#what-ends-up-in-the-html-file)
...
LAST30DAYS_HTML_DIR="${LAST30DAYS_HTML_DIR:-/Users/YOUR_USERNAME/Downloads}"
mkdir -p "$LAST30DAYS_HTML_DIR"
HTML_PATH="${LAST30DAYS_HTML_DIR}/${SLUG}-brief.html"
...
if [ -f "$HTML_PATH" ]; then
  HTML_PATH="${LAST30DAYS_HTML_DIR}/${SLUG}-brief-$(date +%F).html"
fi
...
- **No `${LAST30DAYS_HTML_DIR}` set**: defaults to `/Users/YOUR_USERNAME/Downloads/` per the SKILL.md `Configuration` section. Raw `.md` research still uses `LAST30DAYS_MEMORY_DIR`.
