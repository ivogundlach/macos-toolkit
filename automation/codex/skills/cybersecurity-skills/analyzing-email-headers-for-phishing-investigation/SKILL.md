...
description: >-
  Use when Ivo asks to verify suspicious email headers, sender authenticity,
  SPF/DKIM/DMARC alignment, delivery path, relay chain, spoofing evidence, or
  phishing evidence from raw headers in owned, lab, defensive, educational, or
  explicitly authorized context. Trigger on phrases like "analyze email headers",
  "check SPF DKIM DMARC", "verify this sender", "trace this email's delivery
  path", or "is this email spoofed from the headers". Do not use for generic
  malicious URL behavior or broad cybersecurity work.
license: Apache-2.0
metadata:
...
- When the key evidence is the email header chain, not the clicked URL behavior

Related skills:
- Use `local-read-connectors` when the source message is in Apple Mail or local mail metadata.
- Use `analyzing-malicious-url-with-urlscan` when the task is URL rendering, redirect, DOM, screenshot, or reputation enrichment.
- Use `web-research` only for current public sender-domain, infrastructure, or threat-context facts.
...
curl -s "https://api.abuseipdb.com/api/v2/check?ipAddress=${SENDING_IP}" \
   -H "Key: YOUR_API_KEY" -H "Accept: application/json" | python3 -m json.tool  # gitleaks:allow
```
...
curl -s "https://www.virustotal.com/api/v3/domains/${SENDER_DOMAIN}" \
   -H "x-apikey: YOUR_VT_API_KEY" | python3 -m json.tool  # gitleaks:allow
