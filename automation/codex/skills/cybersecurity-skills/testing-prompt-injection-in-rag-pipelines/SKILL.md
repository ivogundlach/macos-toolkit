...
description: Red-team a RAG pipeline for indirect prompt injection and retrieval/embedding-space poisoning — garak, Promptfoo RAG plugins, PyRIT, plus an offline cosine-similarity poisoning PoC. Trigger on "test my RAG for injection", "poisoned document in my knowledge base", "indirect injection through retrieval". For vector-store confidentiality use assessing-vector-and-embedding-weaknesses; for detection pipelines use detecting-prompt-injection. Authorized use only.
license: Apache-2.0
metadata:
...
When Ivo asks for a saved report or structured findings, use `assets/template.md` as the report skeleton. Otherwise summarize the RAG injection test setup, evidence, and mitigations inline.
