...
description: Assess the security of a vector store / embedding layer in a RAG system (OWASP LLM08) — embedding inversion, membership inference, cross-tenant leakage, and corpus/index poisoning. Trigger on "vector store security", "embedding inversion", "is my RAG index leaking", "Qdrant/Pinecone/Weaviate/Chroma security". For prompt-injection through retrieval use testing-prompt-injection-in-rag-pipelines; for training-time poisoning use detecting-data-and-model-poisoning. Defensive/authorized use only.
license: Apache-2.0
metadata:
