...
description: Harden an AI agent's tool-invocation boundary (defensive) — deny-by-default tool allowlists, argument-schema validation, scoped/STS identity per tool, policy gates, human-in-the-loop for high-risk calls, and audit logging. Trigger on "secure my agent's tool calls", "agent tool allowlist", "least-privilege tool access", "tool-call security". For content/output guardrails use defending-llms-with-guardrails.
license: Apache-2.0
metadata:
