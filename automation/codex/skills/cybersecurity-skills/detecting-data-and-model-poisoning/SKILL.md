...
description: Detect training-time data poisoning and model backdoors (MITRE ATLAS AML.T0020) using ART activation-clustering / spectral signatures, Cleanlab label-error detection, and supply-chain integrity checks. Trigger on "data poisoning", "model backdoor", "is my training set poisoned", "ART/Cleanlab". For inference-time RAG/vector attacks use assessing-vector-and-embedding-weaknesses. Defensive/authorized use only.
license: Apache-2.0
metadata:
