...
description: Scan an LLM for vulnerabilities with NVIDIA garak — enumerate probes (jailbreak, toxicity, leakreplay, encoding), target HuggingFace/OpenAI/REST models, read hit-rate reports, mitigate and re-test. Trigger on "garak", "scan my LLM for vulnerabilities", "run a probe suite". For multi-turn campaigns use orchestrating-llm-attacks-with-pyrit; for CI gating use continuous-llm-red-teaming-with-promptfoo. Authorized use only.
license: Apache-2.0
metadata:
...
When Ivo asks for a saved report or structured findings, use `assets/template.md` as the report skeleton. Otherwise summarize the garak run, hit rates, and next actions inline.
