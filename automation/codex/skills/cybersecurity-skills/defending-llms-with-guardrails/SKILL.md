...
description: Build runtime guardrails for an LLM/RAG/agent app — input/output safety rails, jailbreak/prompt-injection blocking, PII detection & redaction, topic restriction, and structured-output validation. Trigger on "add guardrails", "NeMo Guardrails", "Llama Guard", "LLM Guard", "Presidio PII redaction", "Guardrails-AI", "block jailbreaks in my chatbot", or "content policy for my LLM app". Defensive/authorized use only. Not for offensive red-teaming (use red-teaming-llms-with-garak) or detection-only pipelines (use detecting-prompt-injection).
license: Apache-2.0
metadata:
...
When Ivo asks for a saved report or structured findings, use `assets/template.md` as the report skeleton. Otherwise summarize the guardrail design and validation results inline.
...
### Step 8: PII redaction with Presidio + structured-output validation with Guardrails-AI

For deterministic PII detection/anonymization (30+ entity types) and JSON-schema-validated output, layer Microsoft Presidio and Guardrails-AI alongside the scanners above.

```bash
pip install guardrails-ai presidio-analyzer presidio-anonymizer spacy
...
def redact_pii(text, entities=("PERSON","EMAIL_ADDRESS","PHONE_NUMBER","US_SSN","CREDIT_CARD")):
    results = analyzer.analyze(text=text, entities=list(entities), language="en")
    return anonymizer.anonymize(text=text, analyzer_results=results).text
```

Drive the layers from a JSON content policy so blocked topics/patterns/PII categories live in config, not code:
...
Deploy the combined stack as request/response middleware (validate input → call LLM → validate output), returning a `blocked_reason` or `fallback_response` on failure and keeping the pipeline under ~200ms for input-only validation. Add Guardrails-AI validators when the response must satisfy a JSON schema before reaching downstream systems.
...
| Microsoft Presidio | PII detection & anonymization (30+ entity types) | https://github.com/microsoft/presidio |
| Guardrails AI | Structured-output / JSON-schema validation | https://github.com/guardrails-ai/guardrails |
