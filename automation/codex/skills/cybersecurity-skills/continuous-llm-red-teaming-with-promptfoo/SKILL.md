...
description: Set up continuous CI/CD LLM red-teaming with Promptfoo (+ DeepTeam), mapped to OWASP LLM Top 10 / MITRE ATLAS — generate adversarial test cases and gate on a regression threshold in GitHub Actions. Trigger on "promptfoo", "continuous/CI red-teaming", "regression-test my LLM for jailbreaks". For one-off scans use red-teaming-llms-with-garak; for multi-turn campaigns use orchestrating-llm-attacks-with-pyrit. Defensive/authorized use only.
license: Apache-2.0
metadata:
