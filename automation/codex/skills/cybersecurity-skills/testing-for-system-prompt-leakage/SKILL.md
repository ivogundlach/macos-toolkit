...
description: Test an LLM app for system-prompt / secret leakage (OWASP LLM07) — manual extraction payloads plus garak leakreplay/promptinject and the Promptfoo prompt-extraction plugin; confirm no secrets live in the prompt. Trigger on "is my system prompt leaking", "prompt-extraction test", "system prompt leakage". Distinct from injection detection (detecting-prompt-injection) and RAG injection (testing-prompt-injection-in-rag-pipelines). Authorized use only.
license: Apache-2.0
metadata:
