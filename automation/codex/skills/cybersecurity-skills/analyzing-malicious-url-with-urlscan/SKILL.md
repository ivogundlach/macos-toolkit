...
description: >-
  Use when Ivo asks to enrich, inspect, or investigate a suspicious URL with
  URLScan, rendered-page evidence, screenshots, DOM content, redirect chains,
  HTTP transactions, JavaScript behavior, contacted infrastructure, or URL
  reputation in owned, lab, defensive, educational, or explicitly authorized
  context. Trigger on phrases like "scan this URL with URLScan", "analyze this
  suspicious link", "check this redirect chain", "what infrastructure does this
  URL hit", or "is this URL phishing". Do not use for email-header
  authentication or broad cybersecurity work.
license: Apache-2.0
metadata:
...
- When a suspicious URL needs URLScan rendering, redirect, screenshot, DOM, HAR, or infrastructure enrichment
- When checking whether a link behaves like phishing, credential harvesting, scam, malware delivery, or malicious redirect infrastructure
- When extracting IOCs from URLScan results for defensive blocking or reporting
- When URL behavior is the key evidence, not email-header sender authentication

Related skills:
- Use `analyzing-email-headers-for-phishing-investigation` when the source evidence is raw email headers, SPF/DKIM/DMARC alignment, sender authenticity, or relay-chain verification.
- Use `web-research` for current public reputation or takedown context only after the deterministic URLScan evidence is captured.
