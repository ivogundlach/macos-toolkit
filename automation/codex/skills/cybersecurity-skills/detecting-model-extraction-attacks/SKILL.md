...
description: Detect model-extraction / model-stealing that abuses an inference API (MITRE ATLAS AML.T0024) — high-volume or decision-boundary query patterns, output-entropy monitoring, watermarking, and rate/anomaly defenses. Trigger on "model extraction", "someone stealing my model via the API", "query-pattern anomaly detection for an ML endpoint". Defensive/authorized use only.
license: Apache-2.0
metadata:
